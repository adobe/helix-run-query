...
group_all_events_daily AS (
  ...
  FROM all_raw_events
  GROUP BY id, hostname, trunc_date
),
...
pageviews AS (
  ...
  LEFT JOIN all_raw_events
    ON
      ...
      AND group_all_events_daily.trunc_date = all_raw_events.trunc_date
...
apicalls AS (
  ...
  GROUP BY id, url, hostname, weight, trunc_date
),
...
time_series AS (
  ...
  FROM all_data
  GROUP BY hostname, url, contenttype, weight, trunc_date
)
