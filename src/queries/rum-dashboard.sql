...
current_truncated_rum_by_url AS (
    SELECT
        CAST(
            SUM(ranked.lcpgood * pageviews) / SUM(pageviews) AS INT64
        ) AS lcpgood,
        CAST(
            SUM(ranked.fidgood * pageviews) / SUM(pageviews) AS INT64
        ) AS fidgood,
        CAST(
            SUM(ranked.clsgood * pageviews) / SUM(pageviews) AS INT64
        ) AS clsgood,
        CAST(
            SUM(ranked.lcpbad * pageviews) / SUM(pageviews) AS INT64
        ) AS lcpbad,
        CAST(
            SUM(ranked.fidbad * pageviews) / SUM(pageviews) AS INT64
        ) AS fidbad,
        CAST(
            SUM(ranked.clsbad * pageviews) / SUM(pageviews) AS INT64
        ) AS clsbad,
        CAST(
            SUM(ranked.avglcp * pageviews) / SUM(pageviews) AS INT64
        ) AS avglcp,
        CAST(
            SUM(ranked.avgfid * pageviews) / SUM(pageviews) AS INT64
        ) AS avgfid,
...
FROM (
        SELECT
            ...
            COALESCE(
                current_truncated_rum_by_url.url,
                previous_truncated_rum_by_url.url
            ) AS url,
            COALESCE(
                current_truncated_rum_by_url.pageviews, 0
            ) - COALESCE(
                previous_truncated_rum_by_url.pageviews, 0
            ) AS pageviews_diff
        FROM
            current_truncated_rum_by_url
        FULL OUTER JOIN previous_truncated_rum_by_url
            ON
