...
steps AS (
    SELECT
        ...
        TIMESTAMP_DIFF(
            visittime,
            LAG(visittime) OVER(PARTITION BY url ORDER BY visittime),
            DAY
        ) AS step
    FROM urldays
),
...
chains AS (
    SELECT
        ...
        COUNTIF(
            step = 1
        ) OVER(PARTITION BY url ORDER BY visittime) AS chainlength
    FROM steps
),
            ...
            (
                ...
                FROM
                    (
                        ...
                        FROM
                            quintiletable
                        WHERE quintiletable.cls <= chained.cls
                    )
            ) AS clsscore,
            (
                SELECT MAX(num)
                FROM
                    (
                        ...
                        FROM
                            quintiletable
                        WHERE quintiletable.lcp <= chained.lcp
                    )
            ) AS lcpscore,
            (
                SELECT MAX(num)
                FROM
                    (
                        ...
                        FROM
                            quintiletable
                        WHERE quintiletable.fid <= chained.fid
                    )
            ) AS fidscore,
            (
                SELECT MIN(num)
                FROM
                    (
                        ...
                        FROM
                            quintiletable
                        WHERE chained.load >= quintiletable.load
                    )
            ) AS loadscore,
            (
                SELECT MIN(num)
                FROM
                    (
                        ...
                        FROM
                            quintiletable
                        WHERE chained.click >= quintiletable.click
                    )
            ) AS clickscore,
            (
                SELECT MIN(num)
                FROM
                    (
                        ...
                        FROM
                            quintiletable
                        WHERE chained.reach >= quintiletable.reach
                    )
            ) AS reachscore,
            (
                SELECT MIN(num)
