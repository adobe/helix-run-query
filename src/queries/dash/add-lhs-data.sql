...
IF EXISTS (
  ...
  WHERE
    ...
    CALL helix_external_data.ADD_LHS_DATA(
      @url,
      @perf_score,
      @acc_score,
      @bp_score,
...
END IF;
select * from `helix_external_data.lhs_spacecat` where url = @url and time = @time and audit_ref = @audit_ref and device_type = @device_type