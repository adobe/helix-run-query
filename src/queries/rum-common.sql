...
--- successes: 270
--- zscore: 1.96
CREATE OR REPLACE FUNCTION helix_rum.CALC_BINOMIAL_DISTRIBUTION_MARGIN_OF_ERROR_TEMP(weight NUMERIC, successes NUMERIC, zscore NUMERIC) 
  RETURNS NUMERIC
  AS (
    ...
    --- The z-score for the confidence level: With a 95 percent confidence interval, you have a 5 percent chance of being wrong.
    --- Standard Deviation: Measure of the amount of variation of a random variable expected about its mean
    CAST(COALESCE(zscore, 
      CASE
        ...
        ELSE 1.96
      END
    ) * (weight * SQRT(successes)) AS NUMERIC)
  );
